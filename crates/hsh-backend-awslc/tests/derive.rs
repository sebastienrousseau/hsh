...
fn rfc6070_sha256_test_vector_1() {
    ...
    // a known fixed input.
    let out = pbkdf2_derive(b"password", b"salt", Prf::Sha256, 1, 32).unwrap();
    let expected: [u8; 32] = [
    ...
    assert_eq!(out, expected, "PBKDF2-HMAC-SHA-256 RFC vector mismatch");
}

#[test]
fn rfc6070_sha256_2_iterations() {
    let out = pbkdf2_derive(b"password", b"salt", Prf::Sha256, 2, 32).unwrap();
    let expected: [u8; 32] = [
    ...
    assert_eq!(out, expected, "PBKDF2-HMAC-SHA-256 2-iter vector mismatch");
}
...
fn sha512_smoke() {
    ...
    let out1 = pbkdf2_derive(b"password", b"NaCl", Prf::Sha512, 1000, 64).unwrap();
    let out2 = pbkdf2_derive(b"password", b"NaCl", Prf::Sha512, 1000, 64).unwrap();
    assert_eq!(out1.len(), 64);
    assert_eq!(out1, out2);
    // Different salt produces different output.
    let out3 = pbkdf2_derive(b"password", b"NaCl2", Prf::Sha512, 1000, 64).unwrap();
    assert_ne!(out1, out3);
}
...
fn dk_len_is_honoured() {
    for &len in &[1usize, 16, 32, 48, 64, 128] {
        let out =
            pbkdf2_derive(b"password", b"salt", Prf::Sha256, 1000, len).unwrap();
