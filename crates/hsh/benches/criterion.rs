...
//!    minimum parameters for each algorithm.
...
fn owasp_policy(primary: PrimaryAlgorithm) -> Policy {
    PolicyBuilder::from_preset(&Policy::owasp_minimum_2025())
        .primary(primary)
        .build()
        .expect("OWASP policy")
...
fn bench_hash_owasp_2025(c: &mut Criterion) {
    ...
    let argon_policy = owasp_policy(PrimaryAlgorithm::Argon2id);
    ...
    let bcrypt_policy = owasp_policy(PrimaryAlgorithm::Bcrypt);
    ...
    let scrypt_policy = owasp_policy(PrimaryAlgorithm::Scrypt);
